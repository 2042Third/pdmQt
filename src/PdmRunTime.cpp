...
void PdmRunTime::setupCommands() {
  ...
  commandMap["note_new_note"] = std::make_unique<PDM::pdm_command>("note_new_note", [this](){
    PDM::pdm_qt_net::client_action_note_create(this);
  });
  commandMap["note_refresh_list"] = std::make_unique<PDM::pdm_command>("note_refresh_list", [this](){
    user_data->removeAllFromNoteList(wt.userinfo.email);
    PDM::pdm_qt_net::client_action_note_heads(this);
  });
}
...
void PdmRunTime::updateNoteToServer(int noteid) {// Update the note through network
  emit logc_std("Updating note through network, noteid: " + std::to_string(noteid), "blue");
  PDM::NoteMsg n ;
  user_data->getNote(noteid, &n);
  emit logc_std("Local note head: " + n.head, "blue");
  emit logc_std("Local note content: " + n.content, "blue");
